      ...
      for k, file in enumerate(files):
        if file.endswith('T1w.nii.gz'):
          ...
          for sigma_W in listw:#, (sigmaA,180), (sigmaA*1000,180)]:
            ...
            for i in range(20):
              opt_state, train_loss = train(loss,x_train, y_train, opt_state, key, nIter = ite, batch_size = x_train.shape[0])
              Y_preds.append(forward_passJJ(x_train, get_params(opt_state)).reshape(size))
            Y_preds = np.array(Y_preds)
            with open('results/tEstimation/D-ej-'+tipo2+'-lr'+str(lr)+'-'+str(k)+'.txt', 'w') as f:
...
if __name__ == "__main__":
  ...
  v = np.pi
  for lr in [5*1e-4]:
    learnigspeed("3d", [22.5*v] ,25, lr/10)

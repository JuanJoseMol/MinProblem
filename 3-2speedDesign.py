...
from randomSampling import sample_from_pdf_rejection, sample2d, sample3d, pdfHuman, pdfNumbers, pdfBrain, pdf3d, pdfPDE3d, pdfPDEbrain, pdfPDEhuman, pdfPDEnumbers, pdfPDE, pdfExp, pdfCons
...
def init_params_DesignEDPnumbers(layers, t, key, sigma_W, sigma_a, s=0):
  Ws = []
  Ws.append(sample2d(layers[1], lambda x,y: pdfPDEnumbers(x,y,t) ,sigma_a, s=s).reshape(layers[0], layers[1])*sigma_W)
  key, subkey = random.split(key)
  Ws.append(random.normal(subkey, (layers[1]*2, layers[-1]))*sigma_a)

  return Ws

def init_params_DesignEDPbrain(layers, t, key, sigma_W, sigma_a, s=0):
  Ws = []
  Ws.append(sample2d(layers[1], lambda x,y: pdfPDEbrain(x,y,t),sigma_a, s=s).reshape(layers[0], layers[1])*sigma_W)
  key, subkey = random.split(key)
  Ws.append(random.normal(subkey, (layers[1]*2, layers[-1]))*sigma_a)

  return Ws

def init_params_DesignEDPhuman(layers, t, key, sigma_W, sigma_a, s=0):
  Ws = []
  Ws.append(sample2d(layers[1], lambda x,y: pdfPDEhuman(x,y,t),sigma_a, s=s).reshape(layers[0], layers[1])*sigma_W)
  key, subkey = random.split(key)
  Ws.append(random.normal(subkey, (layers[1]*2, layers[-1]))*sigma_a)

  return Ws

def init_params_DesignEDP3d(layers, t, key, sigma_W, sigma_a, s=0):
  
  Ws = []
  Ws.append(sample3d(layers[1], lambda x,y, z: pdfPDE3d(x,y,z,t), sigma_a, s=s).reshape(layers[0], layers[1])*sigma_W)
  key, subkey = random.split(key)
  Ws.append(random.normal(subkey, (layers[1]*2, layers[-1]))*sigma_a)
...
def load(tipo2):
  ...
  if tipo2 == "brain":
    #../../../
    import nibabel as nib
    A = nib.load(file_path+"sub-r001s001_ses-1_space-MNI152NLin2009aSym_T1w.nii").get_fdata()
  ...
  if tipo2 == "human":
    from skimage.color import rgb2gray
    #../../../
    im = Image.open(file_path+'1 (107).jpg')
  ...
  if tipo2 == "3d":
      ...
      im = resize(onp.swapaxes(nib.load(file_path+"sub-r001s027_ses-1_space-MNI152NLin2009aSym_T1w.nii").get_fdata()[7:187,16:216,5:185], 0, 1), (50,45,45))
      ...
      #im =  resize(np.swapaxes(A, 0, 1), (50,45,45))
...
def learnigspeed(tipo2, listw,ite, lr):
  ...
  for i, sigma_W in enumerate(listw):#, (sigmaA,180), (sigmaA*1000,180)]:
      ...
      if tipo2 == "brain":
        params = init_params_DesignEDPbrain(layers, "F", key, sigma_W, sigmaA)
      if tipo2 == "numbers":
        params = init_params_DesignEDPnumbers(layers, "F", key, sigma_W, sigmaA)
      if tipo2 == "human":
        params = init_params_DesignEDPhuman(layers, "F", key, sigma_W, sigmaA)
      if tipo2 == "3d":
        params = init_params_DesignEDP3d(layers, "F", key, sigma_W, sigmaA)
      ...
      """
      if tipo2 == "brain":
        params = init_params_DesignEDPbrain(layers, "NoU", key , sigma_W, sigmaA)
      if tipo2 == "numbers":
        params = init_params_DesignEDPnumbers(layers, "NoU", key, sigma_W, sigmaA)
      if tipo2 == "human":
        params = init_params_DesignEDPhuman(layers, "NoU", key, sigma_W, sigmaA)
      if tipo2 == "3d":
        params = init_params_DesignEDP3d(layers, "NoU", key, sigma_W, sigmaA)

      opt_init, opt_update, get_params = optimizers.adam(lr)
      opt_state = opt_init(params)

      #for i in range(10):
      opt_state_designEDP1, train_loss_designEDP1 = train(loss,x_train, y_train, opt_state, key, nIter = ite, batch_size = x_train.shape[0])

      LE1.append(train_loss_designEDP1)
      """
      print (np.array(LU).shape)
      with open('speedres/1FUni'+tipo2+'lr'+str(lr)+'-w'+str(sigma_W)+'a'+str(sigmaA.round(3))+'.txt', 'w') as f:
        onp.savetxt(f, np.array(train_loss_normal))
      with open('speedres/1FConst'+tipo2+'lr'+str(lr)+'-w'+str(sigma_W)+'a'+str(sigmaA.round(3))+'.txt', 'w') as f:
        onp.savetxt(f, np.array(train_loss_DCons))
      with open('speedres/1FPDE'+tipo2+'lr'+str(lr)+'-w'+str(sigma_W)+'a'+str(sigmaA.round(3))+'.txt', 'w') as f:
        onp.savetxt(f, np.array(train_loss_designEDP))
      """
      with open('speedres/FPDENoU'+tipo2+'lr'+str(lr)+'-w'+str(sigma_W)+'a'+str(sigmaA.round(3))+'.txt', 'w') as f:
        onp.savetxt(f, np.array(train_loss_designEDP1))
      """
...
if __name__ == "__main__":
  ...
  learnigspeed("human", [70*v] ,1000, 1e-3)

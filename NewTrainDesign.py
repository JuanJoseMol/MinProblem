...
# Bisection method
def bisection_method(g_array, T, tipo2, tol=1e-15, max_iter=100):
    a, b = 1e-14, 1e15 # Define the search interval
...
def sample2d(n_samples, func, key, batch_size=40000):
    sampled_x, sampled_y = [], []

    while len(sampled_x) < n_samples:
        key, kx, ky, kr = random.split(key, 4)

        samples_x = random.uniform(kx, (batch_size,), minval=-1, maxval=1)
        samples_y = random.uniform(ky, (batch_size,), minval=-1, maxval=1)

        pdf_vals = func(samples_x, samples_y)

        # rejection step
        max_val = np.max(pdf_vals)
        rand_vals = random.uniform(kr, (batch_size,)) * max_val
        accepted = rand_vals < pdf_vals

        sampled_x.extend(onp.array(samples_x[accepted]))
        sampled_y.extend(onp.array(samples_y[accepted]))

    # Truncate to exactly n_samples
    return onp.stack([
        onp.array(sampled_x[:n_samples]),
        onp.array(sampled_y[:n_samples])
    ])




def sample3d(n_samples, func, key, batch_size=40000):
    sampled_x, sampled_y, sampled_z = [], [], []

    while len(sampled_x) < n_samples:
        key, kx, ky, kz, kr = random.split(key, 5)

        samples_x = random.uniform(kx, (batch_size,), minval=-1, maxval=1)
        samples_y = random.uniform(ky, (batch_size,), minval=-1, maxval=1)
        samples_z = random.uniform(kz, (batch_size,), minval=-1, maxval=1)

        pdf_vals = func(samples_x, samples_y, samples_z)
        ...
        sampled_x.extend(onp.array(samples_x[accepted]))
        sampled_y.extend(onp.array(samples_y[accepted]))
        sampled_z.extend(onp.array(samples_z[accepted]))

    # Truncate to exactly n_samples
    return onp.stack([
        onp.array(sampled_x[:n_samples]),
        onp.array(sampled_y[:n_samples]),
        onp.array(sampled_z[:n_samples])
    ])
...
def learnigspeed(tipo2, listw,ite, lr, T, Zero):
  if Zero == True:
    text = "Z"
  else:
    text = ""
        ...
        if file.endswith('T1w.nii.gz'):
          ...
          for sigma_W in listw:#, (sigmaA,180), (sigmaA*1000,180)]:
            ...
            def pdf3d(z1, z2, z3):
              ...
              # Raw density
              if Zero == False:
                c = np.min(np.log(g/x_root) / (2*T))
              else:
                c = 0
              
              rhotemp = np.where(g >= x_root, np.log(g/x_root) / (2*T), c)
              norm = np.sum(rhotemp)*8/onp.size(g)
              rhotemp = rhotemp/norm
            ...
            def init_params(layers, key, sigma_W, sigma_a, k):
              Ws = []

              # First weight matrix: samples from 3D pdf
              samples = sample3d(layers[1], lambda x,y,z: pdf3d(x,y,z), key)
              W0 = samples.reshape(layers[0], layers[1]) * sigma_W
              Ws.append(W0)

              # Second weight matrix: Gaussian init
              key, subkey = random.split(key)
              W1 = random.normal(subkey, (layers[1]*2, layers[-1])) * sigma_a
              Ws.append(W1)
            ...
            with open(f'speedres/final/newDesign{text}-{tipo2}-T{T}lr{lr}-{k}.txt', 'w') as f:
          ...
          for sigma_W in listw:#, (sigmaA,180), (sigmaA*1000,180)]:
            ...
            def pdfBrain(z1, z2):
              ...
              if Zero == False:
                ...
                c = 0
              rhotemp = np.where(g >= x_root, np.log(g/x_root) / (2*T), c)
              norm = np.sum(rhotemp)*4/onp.size(g)
              rhotemp = rhotemp/norm
            ...
            def init_params(layers, key, sigma_W, sigma_a):
              Ws = []

              # First weight matrix: samples from 2D pdf
              samples = sample2d(layers[1], lambda x,y: pdfBrain(x,y), key)
              W0 = samples.reshape(layers[0], layers[1]) * sigma_W
              Ws.append(W0)
              ...
              return Ws
            params = init_params(layers, key, listw[0], sigmaA)
        ...
        if file.endswith('.jpg') or file.endswith('.png'):
          ...
          for sigma_W in listw:#, (sigmaA,180), (sigmaA*1000,180)]:
            ...
            def pdfHuman(z1, z2):
              Nx, Ny = g.shape[0:2]
              if Zero == False:
                c = np.min(np.log(g[:, :, 0] /x_root) / (2*T))
              else:
                c = 0
              rhotemp = np.where(g[:, :, 0] >= x_root, np.log(g[:, :, 0] / x_root) / (2*T), c)
              norm = np.sum(rhotemp)*4/onp.size(g)
              rhotemp = rhotemp/norm
            ...
            def init_params(layers, key, sigma_W, sigma_a):
              ...
              # First weight matrix: samples from 2D pdf
              samples = sample2d(layers[1], lambda x,y: pdfHuman(x,y), key)
              W0 = samples.reshape(layers[0], layers[1]) * sigma_W
              Ws.append(W0)
...
if __name__ == "__main__":
  ...
  tbrain, thuman, t3d = 1.5,0.5,0.25
  v = np.pi
  lr = 5*1e-2
  learnigspeed("3d", [22.5*v] ,1000, lr/10, t3d, Cont)
  learnigspeed("human", [75*v] ,1000, lr, thuman, Cont)
  learnigspeed("brain", [90*v] ,1000, lr, tbrain, Cont)
  #tbrain, thuman, t3d = 0.35,0.4,0.0625
  """
  learnigspeed("3d", [22.5*v] ,1000, lr/10, t3d, True)
  learnigspeed("brain", [90*v] ,1000, lr, tbrain, True)
  learnigspeed("human", [75*v] ,1000, lr, thuman, True)

      ...
      for k, file in enumerate(files):
        if file.endswith('T1w.nii.gz'):
          ...
          for sigma_W in listw:#, (sigmaA,180), (sigmaA*1000,180)]:
              ...
              params = init_params_JJ_TGau(layers, key, sigma_W, sigmaA, 22.5*v)
              ...
              with open('speedres/TGau'+tipo2+'lr'+str(lr)+'-w'+str(sigma_W)+'a'+str(sigmaA.round(3))+'-'+str(k)+'.txt', 'w') as f:
              ...
              with open('speedres/Uni'+tipo2+'lr'+str(lr)+'-w'+str(sigma_W)+'a'+str(sigmaA.round(3))+'-'+str(k)+'.txt', 'w') as f:
                onp.savetxt(f, np.array(train_loss_normal))
              
              ######################################################################
              
              for t in [9,24]:#range(1,83):
                ...
                with open('speedres/'+str(t)+'Mean'+tipo2+'lr'+str(lr)+'-w'+str(sigma_W)+'a'+str(sigmaA.round(3))+'-'+str(k)+'.txt', 'w') as f:
                ...
                with open('speedres/'+str(t)+'RhoMean'+tipo2+'lr'+str(lr)+'-w'+str(sigma_W)+'a'+str(sigmaA.round(3))+'-'+str(k)+'.txt', 'w') as f:
                ...
                with open('speedres/'+str(t)+'RhoMediod'+tipo2+'lr'+str(lr)+'-w'+str(sigma_W)+'a'+str(sigmaA.round(3))+'-'+str(k)+'.txt', 'w') as f:
    ...
    for root, dirs, files in os.walk(base_dir):
      ...
      for k, file in enumerate(files):
        if file.endswith('T1w.nii.gz'):
          ...
          key = random.PRNGKey(0)
          for sigma_W in listw:#, (sigmaA,180), (sigmaA*1000,180)]:
              params = init_params_JJ_TGau(layers, key, sigma_W, sigmaA, 90*v)
    ...
    for root, dirs, files in os.walk(base_dir):
      ...
      for k, file in enumerate(files):
        if file.endswith('.jpg') or file.endswith('.png'):
          ...
          for sigma_W in listw:#, (sigmaA,180), (sigmaA*1000,180)]:
              params = init_params_JJ_TGau(layers, key, sigma_W, sigmaA, 75*v)
...
if __name__ == "__main__":
  ...
  #learnigspeed("3d", [3*v, 4*v, 5*v, 6*v, 7*v, 8*v, 9*v, 10*v] ,1000, 1e-3)
  #learnigspeed("human", [5*v,10*v,15*v,20*v,25*v,30*v,35*v,40*v,45*v,50*v] ,1000, 1e-3)
  learnigspeed("brain", [5*v,10*v,15*v,20*v,25*v,30*v,35*v,40*v,45*v,50*v] ,1000, 1e-3)

...
for j, w in enumerate([14*v]):
    for i, t in enumerate(["Uni","Const", "PDE"]): #, "PDENoU"
        with open('speedres/F'+t+'numberslr0.0001-w'+str(w)+'a0.02.txt', 'r') as f:
            u = onp.loadtxt(f)
            print (u.shape)
            #ma.append(np.max(u))
            #mi.append(np.min(u))
...
axs[0, 0].set_title('R001s001 ATLAS')
axs[0, 0].axis('off')
size = im.shape
print (size)
#im -= im.mean()
...
for j, w in enumerate([90*v]):
    for i, t in enumerate(["Uni","Const", "PDE"]): #, "PDENoU"
        with open('speedres/1F'+t+'brainlr0.001-w'+str(w)+'a0.012.txt', 'r') as f:
            u = onp.loadtxt(f)
        axs[j+1, 0].plot([i for i in range(u.shape[0])], u, color=colors[i], label = t+' design')
        axs[j+1, 0].set_title('R = '+str(round(w,3)))
    axs[j+1, 0].set_yscale('log')
...
axs[0, 1].imshow(im)
axs[0, 1].set_title('107 HUMAN FACES')
axs[0, 1].axis('off')
im = im/255.0
#im -= im.mean()
size = im.shape
...
for j, w in enumerate([70*v]):
    for i, t in enumerate(["Uni","Const", "PDE"]): #, "PDENoU"
        with open('speedres/1F'+t+'humanlr0.001-w'+str(w)+'a0.012.txt', 'r') as f:
            u = onp.loadtxt(f)
...
axs[0, 2].imshow(mip)
axs[0, 2].set_title('3D R027s001 ATLAS')
axs[0, 2].axis('off')
...
for j, w in enumerate([22.5*v]):
    for i, t in enumerate(["Uni","Const", "PDE"]): #, "PDENoU"
        with open('speedres/1F'+t+'3dlr0.01-w'+str(w)+'a0.012.txt', 'r') as f:
            u = onp.loadtxt(f)
            print (u.shape)
...
fig.legend(handles, labels, loc='lower center', bbox_to_anchor=(0.5, 0.01), ncol=3)
plt.tight_layout(rect=[0, 0.15, 1, 1])
plt.savefig('speedres/TlessConst-PDEloss.pdf')
plt.show()

...
def double_size_repeat(image):
    rows, cols = image.shape
    doubled_image = onp.repeat(onp.repeat(image, 2, axis=0), 2, axis=1)
    return doubled_image


def double_size_fourier(image):
    rows, cols = image.shape
    # Transform to Fourier space
    f_image = fft2(image)
    f_image = fftshift(f_image)  # Shift zero frequency to center

    # Create a new zero-padded Fourier space
    padded_f_image = onp.zeros((2 * rows, 2 * cols), dtype=complex)
    center_row, center_col = rows // 2, cols // 2
    padded_f_image[center_row: center_row + rows, center_col: center_col + cols] = f_image

    # Transform back to spatial domain
    padded_f_image = ifftshift(padded_f_image)  # Reverse shift
    doubled_image = onp.abs(ifft2(padded_f_image))  # Take inverse FFT and use magnitude
    return doubled_image
...
def learnigspeed(tipo2, listw,ite, lr):
  ...
  def train(loss, X, Y, Xtotal, Ytotal, Y2, Y3, opt_state, key, nIter = 10000, batch_size = 10):
      train_loss = []
      dev1 = []
      dev2 = []
      dev3 = []
      ...
      for it in range(nIter):
          ...
          dev1loss = loss(params, Xtotal, Ytotal)
          dev2loss = loss(params, Xtotal, Y2)
          dev3loss = loss(params, Xtotal, Y3)
          if best_loss > dev1loss:
            bestopt = opt_state
            best_loss = dev1loss

          train_loss.append(train_loss_value)
          dev1.append(dev1loss)
          dev2.append(dev2loss)
          dev3.append(dev3loss)
          to_print = "it %i, train loss = %e, dev loss = %f, dev up = %g, dev zero = %a" % (it, train_loss_value, dev1loss, dev2loss, dev3loss)
          print(to_print)
      return bestopt, train_loss, dev1, dev2, dev3
  ...
  if tipo2 == "brain":
    ...
    grilla_y = im2[:,:,None]
    x_train = meshgrid_from_subdiv(im.shape, (-1,1))#grilla_x[::orig_height // new_height, ::orig_width // new_width]
    y_train = im[:,:,None]
    Y2 = double_size_repeat(im)[:,:,None]
    Y3 = double_size_fourier(im)[:,:,None]
    print (im.shape, layers)
    sigmaA = np.sqrt(2/(layers[-1] + layers[-2]))
    ...
    for sigma_W in listw:#, (sigmaA,180), (sigmaA*1000,180)]:
        ...
        opt_state_normal, train_loss_normal, dev1u, dev2u, dev3u = train(loss,x_train, y_train,
                            grilla_x, grilla_y, Y2, Y3,opt_state, key, nIter = ite, batch_size = x_train.shape[0])


        dicresultsu = {
              'train_loss': train_loss_normal,
              'dev_loss': dev1u,
              "dev_upsampling": dev2u,
              "dev_zeropadding": dev3u,
              'best_param': get_params(opt_state_normal)}

        with open('super/'+tipo2+'uniform-lr'+str(lr)+'w-'+str(round(sigma_W,3))+'.pickle', 'wb') as handle:
          pickle.dump(dicresultsu, handle, protocol=pickle.HIGHEST_PROTOCOL)
        ...
        for t in [27]:
          ...
          with open('super/'+tipo2+'Mean-lr'+str(lr)+'w-'+str(round(sigma_W,3))+'.pickle', 'wb') as handle:
          ...
          with open('super/'+tipo2+'Medoid-lr'+str(lr)+'w-'+str(round(sigma_W,3))+'.pickle', 'wb') as handle:
            pickle.dump(dicresults, handle, protocol=pickle.HIGHEST_PROTOCOL)
          ...
          opt_state_RhoMeanBrain, train_loss_RhoMeanBrain, dev1, dev2, dev3  = train(loss,x_train, y_train,
                                            grilla_x, grilla_y, Y2, Y3, opt_state, key, nIter = ite, batch_size = x_train.shape[0])
          ...
          dicresults = {
              ...
              'dev_loss': dev1,
              "dev_upsampling": dev2,
              "dev_zeropadding": dev3,
              'best_param': get_params(opt_state_RhoMeanBrain)}

          with open('super/Low'+tipo2+'RhoMean-lr'+str(lr)+'w-'+str(round(sigma_W,3))+'.pickle', 'wb') as handle:
            pickle.dump(dicresults, handle, protocol=pickle.HIGHEST_PROTOCOL)

          return dicresultsu, dicresults
          """
          #####################
          ...
          with open('super/'+tipo2+'RhoMediod-lr'+str(lr)+'w-'+str(round(sigma_W,3))+'.pickle', 'wb') as handle:
            pickle.dump(dicresults, handle, protocol=pickle.HIGHEST_PROTOCOL)
          """
...
if __name__ == "__main__":
  ...
  res2, res = learnigspeed("brain", [45*v] ,1000, 1e-1)
  #learnigspeed("brain", [45*v] ,1000, 1e-2)
  #learnigspeed("brain", [45*v] ,1000, 1e-3)

  loss = res['train_loss']
  lossdev = res['dev_loss']
  devUp  = res["dev_upsampling"]
  devF  = res["dev_zeropadding"]
  lossdevuni = res2['dev_loss']

  plt.plot([i for i in range(1000)], loss, label = 'train')
  plt.plot([i for i in range(1000)], lossdev, label = 'dev')
  plt.plot([i for i in range(1000)], devUp, label = 'upsampling')
  plt.plot([i for i in range(1000)], devF, label = 'Zeropadding')
  plt.plot([i for i in range(1000)], lossdevuni, label = 'devuni')
  #plt.title('w = '+str(round(w,3))+', lr = '+str(lr))
  plt.yscale("log")

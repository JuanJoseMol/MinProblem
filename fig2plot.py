...
def slope(u):
    return jnp.polyfit(jnp.linspace(0,1,u.shape[0]), jnp.log(jnp.abs(u/u[0])), 1)[0]

func = lambda x: jnp.sin(100*jnp.pi*x)+ jnp.sin(40*jnp.pi*x)

ft = jnp.fft.fft(func(jnp.linspace(-1,1,240)))
...
def plot():
    ...
    for k, tipo in enumerate(["norm", "uni"]):
        ...
        for j, w in enumerate(SW):
            a = np.load(f'old/ej2-w{w}a0.02236068{tipo}.npy')
            a = jnp.array(a)
            

            n = jnp.zeros((100, a.shape[-1]))

            for i in range(100):
                u = vmap(jnp.fft.fft)(a[i, :it, :]) 
                n = n.at[i].set(
                    vmap(slope)(u.T- ft[:,None])
                )

            y = -jnp.sum(n[:, :120], axis=0) / 100.0

            axs[k+1].plot(
                xf, y,
                color=colors[j],
                label=(
                    f'$\\sigma_w = {int(w)}/2\\pi$'
                    if tipo == "norm"
                    else f'$R = {int(w)}/2\\pi$'
                )
            )
        ...
        axs[k+1].legend(loc='upper right',frameon=False,fontsize=12)
    ...
    plt.tight_layout()
    plt.savefig('results/flr-ej2-1d.pdf', bbox_inches='tight')
    plt.show()

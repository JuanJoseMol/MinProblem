...
def calculate_radial_error(images, num_rings):
    ...
    for i in range(num_rings):
        ...
        ring_errors[:, i] = np.sum(np.abs(ring_values), axis=1)/np.sum(mask)#area[i]
...
def calculate_error_spheres(images, num_spheres):
    ...
    for i in range(num_spheres):
        ...
        sphere_errors[:, i] = np.sum(np.abs(shell_values), axis=1)/np.sum(mask)#area[i]
...
for j, w in enumerate(SW):           
        for k in range(n_test):
            ...
            #u = u[:,int(size[0]/2),:]
            u = vmap(np.fft.fft2)(u) - ft#[int(size[0]/2),:]
...
plt.tight_layout()
plt.savefig('results/newNorm'+tipo+'-LR_others_dataset'+str(smo)+'.pdf')
plt.show()

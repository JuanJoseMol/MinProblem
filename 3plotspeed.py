...
axs[0, 0].set_title('R039s002 ATLAS')
axs[0, 0].axis('off')
size = im.shape
print (size)
del A
...
for j, w in enumerate([90*v]):
    for i, t in enumerate(["MeanUni","5MeanConst","10MeanConst","15MeanConst","20MeanConst"]): #, "PDENoU"
        with open('speedres/'+t+'brainlr0.001-w'+str(w)+'a0.012.txt', 'r') as f:
            u = onp.loadtxt(f)
...
axs[0, 1].imshow(im)
axs[0, 1].set_title('1487 HUMAN FACES')
axs[0, 1].axis('off')
im = im/255.0
#im -= im.mean()
size = im.shape
...
for j, w in enumerate([79.5*v]):
    for i, t in enumerate(["MeanUni","0.5MeanConst","1MeanConst","5MeanConst","10MeanConst"]): #, "PDENoU"
        with open('speedres/'+t+'humanlr0.001-w249.75661596038856a0.009000001.txt', 'r') as f:
            u = onp.loadtxt(f)
...
axs[0, 2].imshow(mip)
axs[0, 2].set_title('3D r039s003 ATLAS')
axs[0, 2].axis('off')
...
for j, w in enumerate([22.5*v]):
    for i, t in enumerate(["MeanUni","5MeanConst","10MeanConst","15MeanConst","20MeanConst"]): #, "PDENoU"
        with open('speedres/'+t+'3dlr0.01-w70.68583470577035a0.010000001-.txt', 'r') as f:
            u = onp.loadtxt(f)
            print (u.shape)
        axs[j+1, 2].plot([i for i in range(1000)], u, color=colors[i], label = t+' design')
...
fig.legend(handles, labels, loc='lower center', bbox_to_anchor=(0.5, 0.01), ncol=3)
plt.tight_layout(rect=[0, 0.15, 1, 1])
plt.savefig('speedres/DiffT-Meanloss.pdf')
plt.show()

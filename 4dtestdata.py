...
def learnigspeed(tipo2, listw,ite, lr):
  ...
  if tipo2 == "3d":
    ...
    for k, gz_path in enumerate(all_files):
      if k>20:
        break
      gz_path = os.path.join(root, file)
      ...
      for sigma_W in listw:#, (sigmaA,180), (sigmaA*1000,180)]:
        ...
        params = init_params_RhoMean3d(layers, 50, key, sigma_W, sigmaA)
        ...
        with open('results/tEstimation/data-Dej-'+tipo2+'-lr'+str(lr)+'-'+str(k)+'.txt', 'w') as f:
  ...
  if tipo2 == "brain":
    ...
    for k, gz_path in enumerate(all_files):
      ...
      for sigma_W in listw:#, (sigmaA,180), (sigmaA*1000,180)]:
        ...
        params = init_params_RhoMeanBrain(layers, 30, key, sigma_W, sigmaA)
  ...
  if tipo2 == "human":
    ...
    for k, gz_path in enumerate(all_files):
      ...
      for sigma_W in listw:#, (sigmaA,180), (sigmaA*1000,180)]:
        ...
        params = init_params_RhoMeanHuman(layers, 30, key, sigma_W, sigmaA)
...
if __name__ == "__main__":
  ...
  v = np.pi
  for lr in [5*1e-2]:
    learnigspeed("3d", [22.5*v] ,10, lr/10)
    #learnigspeed("human", [75*v] ,1, lr)
    #learnigspeed("brain", [90*v] ,1, lr)

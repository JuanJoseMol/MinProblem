    ...
    for root, dirs, files in os.walk(base_dir):
      ...
      for k, file in enumerate(files):
        ...
        if file.endswith('T1w.nii.gz'):
          ...
          params = init_params_JJ(layers, key, 30*np.pi, sigmaA)
          ...
          dicresults = {'loss': train_loss_normal,
                        'params' : get_params(opt_state_normal)}
          with open('speedres/1ejbrainNormal.pickle', 'wb') as handle:
            pickle.dump(dicresults, handle, protocol=pickle.HIGHEST_PROTOCOL)

          """
          ######################################################################

          params = init_params_JJ_uni(layers, key, 90*np.pi, sigmaA)

          opt_init, opt_update, get_params = optimizers.adam(lr)
          opt_state = opt_init(params)

          opt_state_uni, train_loss_uni = train(loss,x_train, y_train, opt_state, key, nIter = ite, batch_size = x_train.shape[0])


          dicresults = {'loss': train_loss_uni,
                        'params' : get_params(opt_state_uni)}
          with open('speedres/1ejbrainUni.pickle', 'wb') as handle:
            pickle.dump(dicresults, handle, protocol=pickle.HIGHEST_PROTOCOL)
          ...
          params = init_params_RhoMeanBrain(layers, 13, key, 90*np.pi, sigmaA)
          ...
          dicresults = {'loss': train_loss_RhoMeanBrain,
                        'params' : get_params(opt_state_RhoMeanBrain)}

          with open('speedres/1ejbrainRhoMean.pickle', 'wb') as handle:
            pickle.dump(dicresults, handle, protocol=pickle.HIGHEST_PROTOCOL)
          """

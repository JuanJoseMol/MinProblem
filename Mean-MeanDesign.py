...
from randomSampling3 import tablepdf3dRhoMean, tablepdfBrainRhoMean, tablepdfHumanRhoMean
...
def init_params_RhoMeanHuman(layers, t, key, sigma_W, sigma_a, s=0):
  Ws = []
  Ws.append(sample2d(layers[1], lambda x,y: tablepdfHumanRhoMean(x,y,t), sigma_W, s=s).reshape(layers[0], layers[1])*sigma_W)
  key, subkey = random.split(key)
  Ws.append(random.normal(subkey, (layers[1]*2, layers[-1]))*sigma_a)
...
def init_params_RhoMeanBrain(layers, t, key, sigma_W, sigma_a, s=0):
  Ws = []
  Ws.append(sample2d(layers[1], lambda x,y: tablepdfBrainRhoMean(x,y,t), sigma_W, s=s).reshape(layers[0], layers[1])*sigma_W)
  key, subkey = random.split(key)
  Ws.append(random.normal(subkey, (layers[1]*2, layers[-1]))*sigma_a)
...
def init_params_RhoMean3d(layers, t, key, sigma_W, sigma_a, s=0):
  Ws = []
  Ws.append(sample3d(layers[1], lambda x,y,z: tablepdf3dRhoMean(x,y,z,t), sigma_a, s=s).reshape(layers[0], layers[1])*sigma_W)
  key, subkey = random.split(key)
  Ws.append(random.normal(subkey, (layers[1]*2, layers[-1]))*sigma_a)
      ...
      for k, file in enumerate(files):
        if file.endswith('T1w.nii.gz'):
          ...
          for sigma_W in listw:#, (sigmaA,180), (sigmaA*1000,180)]:
              ...
              with open('speedres/sgdGau'+tipo2+'lr'+str(lr)+'-w'+str(sigma_W)+'a'+str(sigmaA.round(3))+'-'+str(k)+'.txt', 'w') as f:
              ...
              with open('speedres/psgdUni'+tipo2+'lr'+str(lr)+'-w'+str(sigma_W)+'a'+str(sigmaA.round(3))+'-'+str(k)+'.txt', 'w') as f:
                onp.savetxt(f, np.array(train_loss_normal))
              """
              ######################################################################
              
              #for t in [1.2]:#range(1,83):
              for t in [1,2]:
                """
                ...
                with open('speedres/Fix'+str(t)+'Mean'+tipo2+'lr'+str(lr)+'-w'+str(sigma_W)+'a'+str(sigmaA.round(3))+'-'+str(k)+'.txt', 'w') as f:
                ...
                with open('speedres/'+str(t)+'Medoid'+tipo2+'lr'+str(lr)+'-w'+str(sigma_W)+'a'+str(sigmaA.round(3))+'-'+str(k)+'.txt', 'w') as f:
                ...
                with open('speedres/sgdN'+str(t)+'RhoMean'+tipo2+'lr'+str(lr)+'-w'+str(sigma_W)+'a'+str(sigmaA.round(3))+'-'+str(k)+'.txt', 'w') as f:
                ...
                with open('speedres/sgd'+str(t)+'RhoMediod'+tipo2+'lr'+str(lr)+'-w'+str(sigma_W)+'a'+str(sigmaA.round(3))+'-'+str(k)+'.txt', 'w') as f:
        ...
        if file.endswith('.jpg') or file.endswith('.png'):
          ...
          for sigma_W in listw:#, (sigmaA,180), (sigmaA*1000,180)]:
              ...
              for t in [1, 2]:
                ...
                params = init_params_DConsHuman(layers, t, key, sigma_W, sigmaA)
...
if __name__ == "__main__":
  ...
  """
  #learnigspeed("3d", [8.55,8.510997196304478] ,1000, lr/10) #[ 4*v, 5*v, 6*v, 7*v, 8*v], 
  #learnigspeed("human", [35.86, 35.86792075220431] ,1000, lr)
  #learnigspeed("brain", [42.21,43.065079948177704] ,1000, lr)

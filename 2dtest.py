...
def init_params_JJ1(layers, key, sigma_W):
  Ws = []
  key, subkey = random.split(key)
  Ws.append(random.normal(subkey, (layers[0], layers[1]))*sigma_W)
  key, subkey = random.split(key)
  std_glorot = np.sqrt(2/(layers[-2] + layers[-1]))
  Ws.append(random.normal(subkey, (layers[1]*2, layers[-1]))*std_glorot)

  return Ws


def init_params_JJ3_uni(layers, key, Wmax, sigma_a):
  Ws = []
  key, subkey = random.split(key)
...
def traintest(tipo2, listw, layers, lr, frame):
  ...
  for sigma_W in listw: 
    ...
    for k in range(1):
      ...
      if tipo == "norm":
        params = init_params_JJ1(layers, key, sigma_W)
        ...
        params = init_params_JJ2_uni(layers, key, sigma_W)#, sigmaA)
  ...
  print(f"Execution time: {end_time - start_time:.6f} seconds")
      ...
      #with open('results/ej1-'+tipo2+'lr'+str(lr)+'m15000-w'+str(round(sigma_W,3))+'.txt', 'w') as f:
      #  onp.savetxt(f, Y_preds.reshape((101,-1)) )
...
if __name__ == "__main__":
  ...
  #traintest("brain",[90*v],[2,m_size,1], 5*1e-2,100) #90 #, 60*v, 90*v
  #traintest("astro",[64*v],[2,m_size,1],5*1e-2,10) #128 , 86*v, 128*v
  #traintest("human",[70*v],[2,m_size,3],5*1e-2,100) #70 , 46*v, 70*v
  traintest("3d",[22.5*v],[3,m_size,1],5*1e-3,100) #22.5 , 15*v, 22.5*v
  #[3,2*m_size,256,256,1]

...
def calculate_radial_error(images, num_rings):
    ...
    return ring_errors[10,:], ring_std[10,:]
...
def calculate_error_spheres(images, num_spheres):
    ...
    return sphere_errors[10,:], sphere_std[10,:]
...
tipo3 = "19octuni-"#"2Nobias4multilayer-norm-"
#ej = data.camera()
ej2 = data.astronaut()
...
if tipo == "norm":
    v = 0.75*np.pi
    SW = [64*v,86*v,128*v]#[21*v,42*v,64*v]##[60,90,120]0.08800001#0.009000001
    A = 0.08800001
if tipo == "uni":
    v = np.pi
    SW = [43*v,86*v,128*v]#[21*v,42*v,64*v]##[60,90,120]0.08800001#0.009000001
    A = 0.009000001
...
for j, w in enumerate(SW):           
        for k in range(n_test):
            ...
            path = tipo3+tipo2+'lr'+str(lr)+'Ns'+str(im.shape[0])+'-w'+str(round(w,3))+'a'+str(A)+'-'+str(k)
        ...
        y = np.mean(np.array([gaussian_filter1d(n1[l],smo)  for l in range(n_test)]), axis=0) 
        y1 = np.mean(np.array([gaussian_filter1d(n2[l],smo)  for l in range(n_test)]), axis=0)

        axs[1, 0].plot(np.linspace(0,(int(size[0]/4)),int(len(y))), y, color = colors[j], label = '$Mean R$ = '+str(int(w)))
        axs[2, 0].plot(np.linspace(0,(int(size[0]/4)),int(len(y))), y1, color = colors[j], label = '$Std R$ = '+str(int(w))) 
...
if tipo == "norm":
    v = 0.75*np.pi
    SW = [45*v,60*v,90*v]#[21*v,42*v,64*v]##[60,90,120]0.08800001#0.009000001
    A = 0.08800001
if tipo == "uni":
    v = np.pi
    SW = [30*v,60*v,90*v]#[21*v,42*v,64*v]##[60,90,120]0.08800001#0.009000001
    A = 0.006
...
for j, w in enumerate(SW):           
        for k in range(n_test):
            ...
            path = tipo3+tipo2+'lr'+str(lr)+'Ns'+str(im.shape[0])+'-w'+str(round(w,3))+'a'+str(A)+'-'+str(k)#0.0050000004-'+str(k)
            with open('results/'+path+'.txt', 'r') as f:
                u = onp.loadtxt(f)
            u = u.reshape((11,size[0],size[1]))
        ...
        axs[2, 1].plot(np.linspace(0,(int(size[0]/4)),int(len(y))), y1, color = colors[j], label = '$Std R$ = '+str(int(w))) 
#plt.ylim([-0.5, 2])
...
if tipo == "norm":
    v = 0.75*np.pi
    SW = [35*v,46*v,70*v]#[21*v,42*v,64*v]##[60,90,120]0.08800001#0.009000001
    A = 0.08800001
if tipo == "uni":
    v = np.pi
    SW = [23*v,46*v,70*v]#[21*v,42*v,64*v]##[60,90,120]0.08800001#0.009000001
    A = 0.009000001
...
for j, w in enumerate(SW):
        ...
        axs[2, 2].plot(np.linspace(0,(int(size[0]/4)),int(len(y))), y1, color = colors[j], label = '$Std R$ = '+str(int(w))) 
        #axs[2, 2].set_ylim(0, 15000)
...
if tipo == "norm":
    v = 0.75*np.pi
    SW = [11.25*v,15*v,22.5*v]#[21*v,42*v,64*v]##[60,90,120]0.08800001#0.009000001
    A = 0.08800001
if tipo == "uni":
    v = np.pi
    SW = [7.5*v,15*v,22.5*v]#[21*v,42*v,64*v]##[60,90,120]0.08800001#0.009000001
    A = 0.008
...
for j, w in enumerate(SW):           
        ...
        axs[2, 3].plot(np.linspace(0,(int(size[0]/4)),int(len(y))), y1, color = colors[j], label = '$std R$ = '+str(int(w))) 
#plt.ylim([-0.5, 2])
axs[1, 3].legend(fontsize=8)
axs[2, 3].legend(fontsize=8)
#axs[1, 3].plot(np.linspace(0,(int(size[0]/4)),int(len(y))), np.array([0 for i in range(int(len(y)))]), color="k")
#plt.tight_layout()
...
plt.tight_layout()
plt.savefig('results/10MeanStdUni.pdf')
plt.show()

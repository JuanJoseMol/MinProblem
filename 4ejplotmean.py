...
def calculate_radial_error(images, num_rings):
    ...
    radii = onp.linspace(0, max_radius, num_rings+1)#onp.logspace(0, max_radius, num_rings+1)
...
def calculate_error_spheres(images, num_spheres):
    ...
    # Calculate the logarithmically spaced radii
    radii = onp.linspace(0, max_radius, num_spheres+1)#onp.logspace(0, max_radius, num_spheres+1)#
    area = onp.pi * (radii[1:]**2 - radii[:-1]**2)
...
def slope(u):
    return np.polyfit(np.linspace(0,1,u.shape[0]), (u), 1)[0]
...
tipo3 = "3Nobias4multilayer-norm-"#'19octuni-' #"2Nobias4multilayer-uni-"#"2Nobias4multilayer-uni-"#'19octnorm-'
#ej = data.camera()
ej2 = data.astronaut()
...
for j, w in enumerate(SW):           
        for k in range(n_test):
            ...
            u = np.log(np.abs(u)/(np.abs(u[0])+0.00000001))#-np.log(np.abs(u)/(np.abs(u[0])))
        ...
        axs[1, 0].plot(np.linspace(0,(int(size[0]/4)),int(len(y)))[start:], y[start:], color = colors[j], label = L+' = '+str(int(w))) #[:int(size[1]/2)]
        if j == 0:
            maximun = None
            for offset in range(4):
                try:
                    maximun = math.floor((y[start + offset:].max() + 0.1) * 10) / 10
                    break  # Stop if a valid maximum is found
                except ValueError:
                    continue
#plt.ylim([-0.5, 2])
...
axs[1, 0].set_ylabel('$FLR$', labelpad=-20, fontsize=16)
...
for j, w in enumerate(SW):           
        ...
        axs[1, 1].plot(np.linspace(0,(int(size[1]/4)),int(len(y)))[start:], y[start:], color = colors[j], label = L+' = '+str(int(w)))
        if j == 0:
            maximun = None
            for offset in range(4):
                try:
...
for j, w in enumerate(SW):
        ...
        for k in range(n_test):
            ...
            #u1 = onp.array(u1)
            #u1[:,92,70] = 0
            u1 = np.log(np.abs(u1)/(np.abs(u1[0])+0.00000001))#-np.log(np.abs(u1)/(np.abs(u1[0])))

            u2 = u[:,:,:,1]
            u2 = vmap(lambda x: np.fft.fftshift(np.fft.fft2(x)))(u2) - ft[:,:,1]
            #u2 = onp.array(u2)
            #u2[:,92,70] = 0
            u2 = np.log(np.abs(u2)/(np.abs(u2[0])+0.00000001))#-np.log(np.abs(u2)/(np.abs(u2[0])))

            u3 = u[:,:,:,2]
            u3 = vmap(lambda x: np.fft.fftshift(np.fft.fft2(x)))(u3) - ft[:,:,2]
            #u3 = onp.array(u3)
            #u3[:,92,70] = 0
            u3 = np.log(np.abs(u3)/(np.abs(u3[0])+0.00000001))#-np.log(np.abs(u3)/(np.abs(u3[0])))
        ...
        axs[1,2].plot(np.linspace(0,(int(size[1]/4)),int(len(y)))[start:], y[start:], color = colors[j], label = L+' = '+str(int(w)))
        if j == 0:
            maximun = None
            for offset in range(4):
                try:
...
axs[1, 2].legend(loc='upper right',frameon=False,fontsize=8)
#axs[1, 2].set_xlim((0,35))
axs[1, 2].set_xticks([1,35])
axs[1, 2].set_xlim([0,35])
...
for j, w in enumerate(SW):           
        for k in range(n_test):
            ...
            #u = onp.array(u)
            #u[:,25,23,23] = 0
            u = np.log(np.abs(u)/(np.abs(u[0])+0.00000001))
            u = calculate_error_spheres(u, r)

            n1[k] = vmap(slope)(u[:it,:].T)#np.abs(u[it,:])
        y = np.mean(np.array([gaussian_filter1d(n1[l],.001)/T  for l in range(n_test)]), axis=0) #np.array(-n1[0])#gaussian_filter1d(-n1[l],10)

        axs[1, 3].plot(np.linspace(0,(int(size[1]/4)),int(len(y)))[start:], y[start:], color = colors[j], label = L+' = '+str(int(w)))
        if j == 0:
            maximun = None
            for offset in range(4):
                try:
...
axs[1, 3].legend(loc='upper right',frameon=False,fontsize=8)
...
plt.tight_layout()
plt.savefig('results/'+str(start)+'-0f-'+tipo3+'-FLRdataset'+str(smo)+'.pdf', bbox_inches='tight')
plt.show()

...
def learnigspeed(tipo2, listw,ite, lr, m, ej):
  ...
  if tipo2 == "3d":
    data = [data2[0.25], data2[0.5], data2[1], data2[3]]#[data2[1], data2[3], data2[5], data2[7]]
  if tipo2 == "brain":
    data = [data2[2], data2[4], data2[6], data2[8]]
  if tipo2 == "human":
    data = [data2[1], data2[3], data2[5], data2[7]]
  
  for T, x_root, p in data:
    ...
    if tipo2 == "brain":
      ...
      def init_params(layers, key, sigma_W, sigma_a, s=0):
        Ws = []
    ...
    #for i in range(10):
    opt_state_design, train_loss_design, sigma_design = train(loss,x_train, y_train, opt_state, key, nIter = ite, batch_size = x_train.shape[0])

    with open('hiper/f3-ej'+ej+'-'+str(m)+'design-'+tipo2+'T'+str(T)+'por'+str(p)+'.txt', 'w') as f:
  ...
  for sigma_W in listw:#T, sigma_W in data1.values():
    ...
    with open('hiper/f3-ej'+ej+'-'+str(m)+'normal-'+tipo2+'sigma'+str(sigma_W)+'.txt', 'w') as f:
...
if __name__ == "__main__":
  ...
  m = 15000
  ej = "1"
  learnigspeed("3d", [22.5*v] ,1000, 0.75*1e-2, m, ej)
